...
2. **保存：** 将每篇文档以 Markdown 格式保存到 `audit/raw/{DOC_ID}.md`（`DOC_ID` 从文档名或 URL 推断，如 `RFC2460`）。
3. **汇总：** 所有文档获取完成后，得到 `audit/raw/` 下的一组 `.md` 文件，供 A.1 逐篇切分。

### A.1 切分章节

对 `audit/raw/` 中的**每一篇**文档，运行辅助脚本确定性地清洗并切分（无 LLM）：

```bash
python .opencode/skills/rfc-audit/scripts/split_rfc.py audit/raw/{DOC_ID}.md audit/ --rfc-id {DOC_ID}
```

脚本会**合并**写入 `audit/rfc_sections.json`——多次运行（每篇文档一次）自动累积，不会覆盖之前的结果。
...
脚本将章节文件写入 `audit/sections/`，JSON 索引骨架写入 `audit/rfc_sections.json`（`title` 和 `content_path` 已填充，`summary` 留空）。
...
将所有分片结果合并到 `audit/code_map.json`：
...
将确认的不一致写入 `audit/inconsistencies.json`：
